    ...
    public class HousekeeperDAO
    {
        ...
        public async Task<List<Housekeeper>> GetAllHousekeepersAsync(int pageNumber, int pageSize)
        {
            ...
            try
            {
                ...
                {
                    list = await context.Housekeeper.Include(i => i.IDVerification).Include(a => a.Account).Where(a => a.Account.RoleID==1 && a.IsVerified).AsNoTracking().Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
                }
            }
            catch (Exception ex)
            {
